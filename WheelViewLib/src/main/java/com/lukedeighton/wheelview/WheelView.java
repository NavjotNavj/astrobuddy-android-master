...
public class WheelView extends View {
    ...
    public static final int VALUE_ANIMATION_DURATION = 2000;
    /**
    ...
    private OnWheelItemSelected onWheelItemSelected;
    ...
    public void setOnWheelItemSelected(OnWheelItemSelected onWheelItemSelected) {
        this.onWheelItemSelected = onWheelItemSelected;
    }
    ...
    public interface OnWheelItemSelectListener {
        ...
        void onWheelItemSettled(WheelView parent, int position);
    ...
    public void setOnWheelItemVisibilityChangeListener(OnWheelItemVisibilityChangeListener listener) {
     ...
     * @see #setMidSelected()
     */
    public void setSelected(final int rawPosition) {
        if (isWheelRotating)
            return;
        ...
        //NOTE : ValueAnimator is used to give smooth rotation effect when a Item is selected from WheelView
        ...
        valueAnimator.setDuration(VALUE_ANIMATION_DURATION);
        valueAnimator.start();
     ...
     * @see #setMidSelected()
     */
    public void setSelectedWithoutAnimation(int rawPosition) {
        if (isWheelRotating)
            return;
        ...
        //NOTE : ValueAnimator is used to give smooth rotation effect when a Item is selected from WheelView
        /*ValueAnimator valueAnimator = ValueAnimator.ofFloat((-1f * getAngle()), (-1f * getAngleForPosition(rawPosition)));
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                setAngle((Float) valueAnimator.getAnimatedValue());
        ...
        if (onWheelItemSelected != null) {
            onWheelItemSelected.onItemSelected(rawPosition);
        }
    ...
    private void setSelectedPosition(int position) {
        ...
        //Log.d("Set Selection", " Anfle : " + mLastTouchAngle + " - " + mDraggedAngle);
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                if (SystemClock.uptimeMillis() - 100 > lastSelectionUpdate) {
                    int adapterPos = getSelectedPosition();
                    // mOnItemSelectListener.onWheelItemSettled(WheelView.this, getWheelItemDrawable(adapterPos), adapterPos);
                }
            }
        }, 600);
    }
    ...
    private void update(float deltaTime) {
        ...
        if (mAngularVelocity != 0f) {
            ...
            // setSelectedWithoutAnimation(getSelectedPosition());
            if (!isItemClicked) {
                // setAngle(getWheelItemAngle());
                setSelectedWithoutAnimation(getSelectedPosition());
            }
            //  mOnItemSelectListener.onWheelItemSettled(WheelView.this, getSelectedPosition());
        }
    }
        ...
        for (int i = offset; i < length; i++) {
            ...
            if (Rect.intersects(sTempRect, mViewBounds)) {
                ...
                if (i == mRawSelectedPosition && mSelectionDrawable != null && !isEmptyItemPosition(i)) {
                   ...
                   /* mSelectionDrawable.setBounds(sTempRect.left + 35, sTempRect.top - 10,
                            sTempRect.right - 35, sTempRect.bottom - 60);*/
