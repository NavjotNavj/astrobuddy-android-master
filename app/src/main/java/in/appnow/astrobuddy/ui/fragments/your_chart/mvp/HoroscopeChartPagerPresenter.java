...
/**
 * Created by sonu on 17:34, 06/06/18
 * Copyright (c) 2018 . All rights reserved.
 */
public class HoroscopeChartPagerPresenter implements BasePresenter {
    private static final String TAG = HoroscopeChartPagerPresenter.class.getSimpleName();
    private final HoroscopeChartPagerView view;
    private final HoroscopeChartPagerModel model;
    public final PreferenceManger preferenceManger;
    private ABDatabase abDatabase;

    private final CompositeDisposable disposable = new CompositeDisposable();

    public HoroscopeChartPagerPresenter(HoroscopeChartPagerView view, HoroscopeChartPagerModel model, PreferenceManger preferenceManger, ABDatabase abDatabase) {
        this.view = view;
        this.model = model;
    ...
    public void setUpViewPager(FragmentManager fragmentManager) {
        if (preferenceManger != null && view != null)
            view.setUpViewPager(fragmentManager, preferenceManger.getUserDetails().getUserProfile().getDob(), preferenceManger.getUserDetails().getUserProfile().getLatlong());
    }
